...
export default function DataCard({ icon, amount, description, isbordered = false, amountDescription, isLoading = false, columns = 1 }: DataCardProps) {
  return (
    <DataCardContent isbordered={isbordered} isLoading={isLoading} columns={columns}>
...
export function DataCardContent({
  ...
  columns = 1,
  ...rest
}: React.HTMLAttributes<HTMLDivElement> & { isbordered: boolean, isLoading?: boolean, columns: number }) {
